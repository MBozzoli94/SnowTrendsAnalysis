...
for (j in 1:length(ms)) {
  ...
  EV_sp_X <- as.data.frame(matrix(nrow=length(EV2_X$Name), ncol=8))
  EV_sp_X[,1:5] <- Meta[rownames(EV2_X),]
  colnames(EV_sp_X) <- c(colnames(Meta),"Color","Pv","Pt")
  
  EV2_X <- EV2_X[,-c(1,6)]
  Pv2_X <- Pv2_X[,-1]
  
  EV_sp_X[,6] <- Var_c[max.col(EV2_X)]
  
  for (i in 1:length(EV_sp_X$Name)) {
    
    EV_sp_X[i,7] <- Pv2_X[i,max.col(EV2_X)[i]]
    
    if (EV_sp_X$Pv[i] > a) {EV_sp_X$Pt[i] <- 0}
    if (EV_sp_X$Pv[i] <= a) {EV_sp_X$Pt[i] <- 15}
...
png(paste0("09_EVAnalysis/Plots/EVAnalysis_3.png"), width=2400, height=1200, res=100)
...
for (j in 1:(length(ms))) {
  ...
  points(EV_sp[[j]]$Longitude, EV_sp[[j]]$Latitude, type="p", pch=EV_sp[[j]]$Pt, col=EV_sp[[j]]$Color, bg=EV_sp[[j]]$Color, cex=4, lwd=3)
  legend("bottomright", legend=Var[-1], pch=0, col=Var_c, pt.cex=3, pt.lwd=3, cex=2, bty="n")
...
#     
#     # P
#     if (EV_sp_X$Pv[i] > a & EV_sp_X$Color[i] == "#3d77ff") {EV_sp_X$Pt[i] <- 0}
#     if (EV_sp_X$Pv[i] <= a & EV_sp_X$Color[i] == "#3d77ff") {EV_sp_X$Pt[i] <- 15}
#     
#     # TMEAN
#     if (EV_sp_X$Pv[i] > a & EV_sp_X$Color[i] == "#33D81B") {EV_sp_X$Pt[i] <- 1}
#     if (EV_sp_X$Pv[i] <= a & EV_sp_X$Color[i] == "#33D81B") {EV_sp_X$Pt[i] <- 16}
#     
#     # NAO
#     if (EV_sp_X$Pv[i] > a & EV_sp_X$Color[i] == "#FF8F00") {EV_sp_X$Pt[i] <- 2}
#     if (EV_sp_X$Pv[i] <= a & EV_sp_X$Color[i] == "#FF8F00") {EV_sp_X$Pt[i] <- 17}
#     
#     # AO
#     if (EV_sp_X$Pv[i] > a & EV_sp_X$Color[i] == "#ff2600") {EV_sp_X$Pt[i] <- 5}
#     if (EV_sp_X$Pv[i] <= a & EV_sp_X$Color[i] == "#ff2600") {EV_sp_X$Pt[i] <- 18}
#     
#   }
#   
#   EV_sp[[j]] <- EV_sp_X
...
#   plot(Hillshade, main=ms[j], cex.main=3, col=gray(0:255/255), legend=F, axes=F, box=T, alpha=0.6)
#   plot(TAA, lwd=1, border="black", add=T)
#   points(EV_sp[[j]]$Longitude, EV_sp[[j]]$Latitude, type="p", pch=EV_sp[[j]]$Pt, col=EV_sp[[j]]$Color, cex=4, lwd=3)
#   legend("bottomright", legend=Var[-1], pch=c(0,1,2,5), col=Var_c, pt.cex=3, pt.lwd=3, cex=2, bty="n")
#   
# }
# 
# dev.off()
