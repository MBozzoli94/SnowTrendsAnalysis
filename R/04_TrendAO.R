...
MAVG_10y <- movavg(Data2$AO, n=120, type="e")

png(paste0("04_TrendAO/Plots/AO_8020.png"), width=1100, height=1000, res=100)
par(mar=c(4.5,4.5,0.5,0.5), mgp=c(3,1,0))

plot(Data2$AO, xaxt="n", yaxt="n", ylim=c(-4.5,4.5), xlim=c(0,length(Data2$Month)+1),
     xlab="Date", ylab="AO", type="h", col=bCol, lwd=1.5, cex.axis=1.6, cex.lab=2)
...
MAVG_10y <- movavg(Data3$AO, n=10, type="e")

png(paste0("04_TrendAO/Plots/AO_8020_S.png"), width=1100, height=1000, res=100)
par(mar=c(4.5,4.5,0.5,0.5), mgp=c(3,1,0))

plot(Data3$AO, xaxt="n", yaxt="n", ylim=c(-2,2), xlim=c(0,length(Data3$Date)+1),
     xlab="Date", ylab="AO", type="h", col=bCol, lwd=3, cex.axis=1.5, cex.lab=1.5)
lines(MAVG_10y, col="black", lwd=3)
axis(side=1, at=ax, labels=ax_lab, las=1, cex.axis=1.5, tck=-0.01, xaxs="i")
axis(side=2, at=round(seq(-2,2,length.out=11),1), labels=round(seq(-2,2,length.out=11),1),
     las=2, cex.axis=1.5, tck=-0.01, xaxs="i")
...
plotCI(seq(1:8), t(Trend[1,]), ui=t(Trend[3,]), li=t(Trend[2,]), gap=0, sfrac=0.01, barcol="grey",
       xaxt="n", yaxt="n", ylim=c(-0.8,0.8), xlim=c(0.5,8.5), xlab=NA, ylab="Trend AO", type="p",
...
axis(side=2, at=seq(-0.8,0.8,0.2), labels=seq(-0.8,0.8,0.2), las=2, cex.axis=1.5, tck=-0.01, xaxs="i")
