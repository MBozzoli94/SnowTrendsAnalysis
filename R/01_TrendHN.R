...
# ##### TABLE V3 #####
# TTab_v3 <- as.data.frame(matrix(nrow=length(ms), ncol=9))
# colnames(TTab_v3) <- rep(c("#","AT","RT"),3)
# rownames(TTab_v3) <- ms
# id_t <- c(1,4,7)
# 
# for (j in 1:length(ms)) {
...
#   for (k in 1:3) {
#     
#     TTab_v3[j,id_t[k]] <- sum(!is.na(TS_s[,j+1][which(Meta$Elevation > ar[k] & Meta$Elevation <= ar[k+1])]))
#     TTab_v3[j,id_t[k]+1] <- round(mean(TS_s[,j+1][which(Meta$Elevation > ar[k] & Meta$Elevation <= ar[k+1])], na.rm=T), 1)
#     TTab_v3[j,id_t[k]+2] <- round(100*(mean(TS_s[,j+1][which(Meta$Elevation > ar[k] & Meta$Elevation <= ar[k+1])]/
#                                             HN_avg[,j+1][which(Meta$Elevation > ar[k] & Meta$Elevation <= ar[k+1])],
#                                             na.rm=T)), 1)
#     
#   }
...
# }
#
# write.table(TTab_v3, "TrendHN_gap_00.csv", sep=";", row.names=T, col.names=T)
